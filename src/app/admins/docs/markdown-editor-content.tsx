...
export default function MarkdownEditorPage() {
    ...
    const [isClient, setIsClient] = useState(false) // Thêm state để kiểm tra client-side

    useEffect(() => {
        setIsClient(true) // Set thành true khi component được mount
    }, [])

    useEffect(() => {
        if (isClient) {
            // Chỉ thực hiện việc lấy searchParams khi đã mount trên client
            setAssignmentId(searchParams.get('assignmentId') || '')
